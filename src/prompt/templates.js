...
export const TEMPLATES = {
  ...
  srp_ui: {
    ...
    contextLabel: 'Captured search.unbxd.io "search"/"category" request context (URL, apiType, params, response counts and field shape — never the catalogue data itself), every captured call with its contentType filter, what the DOM actually rendered, the reviewed {siteKey}_search.js/.css config bundle including how price is rendered, and sdkAssets',
    focus: SELF_DEBUG_PREFIX + SDK_CHECK_PREFIX + `If the engineer is asking where a value on screen comes from (they usually paste the element), read context.valueTrace and follow the chain DOM text → response field → attributesMap alias → template; in agent mode use trace_rendered_value. Never guess the source field from its name. Otherwise, make the API-versus-UI call explicitly and early, using the numbers: responseSummary.numberOfProducts and returnedProductCount versus renderedPage.domProductNodeCounts. If the API returned products but the DOM shows none, it is a rendering/templating problem — go straight to siteConfig.liveConfig: check selectorChecks for any matchCount of 0 (a config element selector that matches nothing is the single most common cause of an empty grid), then products.attributesMap / productAttributes against responseSummary.productFieldNames for a field the template reads but the API doesn't return. If the API returned zero, look at the request params (q for search / p for category, filters, pagination start) and at siteConfig.consistency for a site-key mismatch between bundle, running config and request. Also check for a redirect, didYouMean or an error field, and whether the response parsed as JSON at all. Before calling repeated calls a duplication bug, read allApiCalls[].filter: a tabbed storefront runs one instance per tab with distinct contentType filters, which is by design — pageTab says which tab the page is on and which call was treated as primary. For any price complaint, read siteConfig.priceConfig in the order requested → returned → configured → drawn, and do not blame a mapped-but-missing field unless priceRendererInBundle shows the renderer actually reads it. ` + CONFIG_REVIEW_NOTE,
    output: SHARED_OUTPUT_CONTRACT
  },
